"""Step 3 part 2: generate the canonical overtime interpretation.

This file starts from the intermediate
`*_overtime_clause_classification.json` artifact produced by part 1.
It then:
1. keeps only the clause roles that can create overtime;
2. runs the expert interpretation passes;
3. compares the expert outputs when more than one pass is used;
4. validates the merged rules; and
5. writes the canonical JSON and markdown interpretation artifacts.
"""
...
def generate_overtime_interpretation_from_classifications(
    ...
    for run_index in range(effective_expert_run_count):
        ...
        structured_rules, validation_warnings, _output_text = (
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    """Parse CLI arguments for step 3 part 2."""
    parser = argparse.ArgumentParser(
        description="Generate an overtime interpretation from an existing overtime clause classification JSON."
    )
    parser.add_argument(
        "classification_path",
        nargs="?",
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Path to the intermediate overtime clause classification JSON. "
            "Defaults to the standard step-3 output path beside the interpretation."
        ),
    )
    parser.add_argument(
        "--model",
...
def main() -> None:
    """Run step 3 part 2 from the command line."""
    args = parse_args()
